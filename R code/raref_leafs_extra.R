#Analytic rarefaction (Expected value, Variance and SD for rarefied variables); stepe size for resampling routine is one!
#Code from Torsten Wappler and is analytic rarefaction solution for herbivory data first described and published in Gunkel & Wappler 2015.

mask      <- read.csv("LSmask.csv", header=T) #Read mask- this file defines each DT- herbivory, specialized, gall, mine
ffg_def   <- read.csv("LSffgdef.csv", header=T)$x #file defining FFGs: Boring, Fungal, Seed Predation, Incertae, Oviposition =0, Galling=3, Hole Feeding=4, Mining =5, Margin Feeding =6,  Piercing and Sucking=7, Skeletonization=1, Surface feeding =2, 

inputfilename <- "LaSelva/TW.LS.1902.csv" #initial data matrix; first column is the species (header must be ID); the rest of the columns are the damage types (we go up to 223)
dataname <- "LS_1902" #free to choose 
cutoff <- 2000 #cutoff for resampling data from single species
...
#Constructs a matrix of % of leaves with each functional feeding group
...
#################Convert to pres-abs data
#dataall[,2:223][dataall[,2:223]>0]<-1
